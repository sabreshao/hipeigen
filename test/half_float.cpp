...
#include <Eigen/src/Core/arch/HIP/hcc/Half.h>
...
{
  using Eigen::half_impl::__hip_half;
  ...
  float val1 = float(half(__hip_half(0x3c00)));
  float val2 = float(half(__hip_half(0x3c01)));
  float val3 = float(half(__hip_half(0x3c02)));
  ...
  // Conversion to float.
  VERIFY_IS_EQUAL(float(half(__hip_half(0x0000))), 0.0f);
  VERIFY_IS_EQUAL(float(half(__hip_half(0x3c00))), 1.0f);

  // Denormals.
  VERIFY_IS_APPROX(float(half(__hip_half(0x8001))), -5.96046e-08f);
  VERIFY_IS_APPROX(float(half(__hip_half(0x0001))), 5.96046e-08f);
  VERIFY_IS_APPROX(float(half(__hip_half(0x0002))), 1.19209e-07f);

  // NaNs and infinities.
  VERIFY(!(numext::isinf)(float(half(65504.0f))));  // Largest finite number.
  VERIFY(!(numext::isnan)(float(half(0.0f))));
  VERIFY((numext::isinf)(float(half(__hip_half(0xfc00)))));
  VERIFY((numext::isnan)(float(half(__hip_half(0xfc01)))));
  VERIFY((numext::isinf)(float(half(__hip_half(0x7c00)))));
  VERIFY((numext::isnan)(float(half(__hip_half(0x7c01)))));
  ...
  VERIFY(!(numext::isinf)(half(__hip_half(0x7bff))));
  VERIFY(!(numext::isnan)(half(__hip_half(0x0000))));
  VERIFY((numext::isinf)(half(__hip_half(0xfc00))));
  VERIFY((numext::isnan)(half(__hip_half(0xfc01))));
  VERIFY((numext::isinf)(half(__hip_half(0x7c00))));
  VERIFY((numext::isnan)(half(__hip_half(0x7c01))));
